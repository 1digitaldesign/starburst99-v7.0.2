...
class Starburst99:
    ...
    def run(self):
        ...
        try:
            ...
            # Initialize output writer with proper directory
            self.output_writer = OutputWriter(self.galaxy.model_params.output_directory)
    ...
    def _read_atmosphere_data(self):
        """Read atmosphere/spectrum library data"""
        # Implementation for reading atmosphere data files
        if hasattr(self, 'namfi3'):
            # If namfi3 is just a filename code, build full path
            if len(str(self.namfi3)) <= 3:
                atmosphere_file = self.galaxy.data_dir / "lejeune" / f"lcb97_{self.namfi3}.flu"
            else:
                atmosphere_file = Path(self.namfi3)
        else:
            atmosphere_file = self.galaxy.data_dir / "lejeune" / "lcb97_p00.flu"
        
        if not atmosphere_file.exists():
            self.logger.error(f"Atmosphere data file not found: {atmosphere_file}")
            raise FileNotFoundError(f"Atmosphere data file not found: {atmosphere_file}")
